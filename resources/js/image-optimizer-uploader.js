window.imageOptimizerUploader = (config) => ({
    batch: null,
    queue: [],
    dragActive: false,
    busy: false,
    ...
    async restoreBatch() {
        ...
        const remembered = window.localStorage.getItem('minisystems-image-optimizer-batch');
        ...
        try {
            ...
            window.localStorage.removeItem('minisystems-image-optimizer-batch');
        } finally {
    ...
    applyBatch(batch, notify = true) {
        ...
        window.localStorage.setItem('minisystems-image-optimizer-batch', batch.uuid);
        ...
        if (
            ...
            const message = batch.status === 'completed'
                ? `${batch.completed_files} imágenes fueron optimizadas correctamente.`
    ...
    currentSettings() {
        return {
            profile: this.$wire.profile,
            format: this.$wire.format,
            quality: Number(this.$wire.quality),
            max_width: Number(this.$wire.maxWidth),
            max_height: Number(this.$wire.maxHeight),
            target_kb: this.$wire.targetKb === '' || this.$wire.targetKb === null
                ? null
                : Number(this.$wire.targetKb),
            allow_upscale: Boolean(this.$wire.allowUpscale),
            preserve_transparency: Boolean(this.$wire.preserveTransparency),
        ...
        if (pending.length === 0) {
            this.alert(
                ...
                this.isTerminal(this.batch?.status)
                    ...
                    : 'Espera a que termine la optimización actual.',
            );
            return;
        }
    ...
    async retryProcessing(item) {
        try {
            ...
            this.applyBatch(response.batch, false);
            this.toast('success', 'Reintento enviado', 'La imagen volvió a la cola.');
        } catch (error) {
    ...
    batchStatusText(status) {
        return {
            ...
            processing: 'Optimizando el lote',
