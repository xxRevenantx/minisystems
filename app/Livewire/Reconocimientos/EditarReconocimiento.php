...
class EditarReconocimiento extends Component
{
    ...
    public function actualizarReconocimiento(): void
    {
        ...
        $data = $this->validate([
            'reconocimiento_imagen_id' => 'required|exists:reconocimiento_imagenes,id',
            'reconocimiento_evento_id' => 'nullable|exists:reconocimiento_eventos,id',
            'reconocimiento_tipo_id' => 'nullable|exists:reconocimiento_tipos,id',
            'reconocimiento_a' => 'required|string|min:3|max:255',
            'lugar_obtenido' => 'nullable|string|max:255',
            'descripcion' => 'required|string|max:5000',
            'fecha' => 'required|date',
            'estado' => 'required|in:borrador,revision,aprobado,generado,entregado,cancelado',
            'directivos' => 'required|array|min:1|max:5',
            'directivos.*' => 'exists:directivos,id',
            'delivery_method' => 'nullable|in:impreso,correo,whatsapp,digital,ceremonia',
        ...
        $update = [
            'reconocimiento_imagen_id' => $data['reconocimiento_imagen_id'],
            'reconocimiento_evento_id' => $data['reconocimiento_evento_id'],
            'reconocimiento_tipo_id' => $data['reconocimiento_tipo_id'],
            'reconocimiento_a' => trim($data['reconocimiento_a']),
            'lugar_obtenido' => $data['lugar_obtenido'] ? trim($data['lugar_obtenido']) : null,
            'descripcion' => ReconocimientoHtml::limpiar($data['descripcion']),
            'fecha' => $data['fecha'],
            'estado' => $sustantivo ? 'revision' : $data['estado'],
            'version' => $sustantivo ? $this->reconocimiento->version + 1 : $this->reconocimiento->version,
    ...
    public function enviarCorreo(): void
    {
        ...
        $reconocimiento->update([
            'estado' => 'entregado',
            'generated_at' => $reconocimiento->generated_at ?: now(),
            'delivered_at' => now(),
